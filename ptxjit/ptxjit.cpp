...
void ptxJIT(int argc, char **argv, CUmodule *phModule, CUfunction *phKernel, CUlinkState *lState)
{
    ...
    FILE *ptxFile;
    long ptxFileSize;
    const char *ptxFileName = argv[1]; //TODO: Make sure that this code knows how to parse arguments and grab the correct filename from the list
    ...
    ptxFile = fopen(ptxFileName, "r");
    fseek(ptxFile, 0, SEEK_END);
    ptxFileSize = ftell(ptxFile);
    rewind(ptxFile);

    ptxCode = (char *)malloc(sizeof(char) * ptxFileSize + 1);
    fread(ptxCode, ptxFileSize, 1, ptxFile);
    fclose(ptxFile);
    ptxCode[ptxFileSize] = '\0'; // Just in case... do we need this?

    bool shouldUse32Bit = sizeof(void *)==4;
    printf("Loading %s program\n", ptxFileName);
    if (shouldUse32Bit)
    {
        // PTX May also be loaded from file, as per below.
        myErr = cuLinkAddData(*lState, CU_JIT_INPUT_PTX, (void*)ptxCode, strlen(ptxCode)+1, 0, 0, 0, 0);
    }
    else
    {
        myErr = cuLinkAddData(*lState, CU_JIT_INPUT_PTX, (void*)ptxCode, strlen(ptxCode)+1, 0, 0, 0, 0);
    ...
    checkCudaErrors(cuModuleGetFunction(phKernel, *phModule, "vector_square"));
...
int main(int argc, char **argv)
{
    ...
    int numRegs;
    cuFuncGetAttribute(&numRegs, CU_FUNC_ATTRIBUTE_NUM_REGS, hKernel);
    printf ("numRegs = %d\n", numRegs);
